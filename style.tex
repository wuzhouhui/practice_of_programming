...
(偶然的) reader. What relationship links Singapore, Brunei, Poland and
Italy? Why isn't Italy mentioned in the comment? Since the comment and the
code differ, one of them must be wrong. Maybe both are. The code is what
gets executed and tested, so it's more likely to be right; probably the
comment didn't get updated when the code did. The comment doesn't say
enough about the relationship among the three countries it does mention; if
you had to maintain this code, you would need to know more.
...
clear and simple -- straightforward logic, natural expression, conventional
...
conventions, management edict (法令), or a program, but even if not,
it is best to obey a set of widely shared conventions. We follow
...
transport (明显) as possible. Write the clearest code that does the job.
Use spaces around operators to suggest grouping; more generally, format to
help readability. This is trivial (琐细的) but valuable, like keeping a
neat desk so you can find things. Unlike your desk, your programs are
...
can be used to make the intent clear even when they are not required. The
inner parentheses in the previous example are not necessary, but they don't
hurt, either. Seasoned (经验丰富的) programmers might omit them, because
the relational operators(\verb"< <= == != >= >") have higher precedence
than the logical operators(\verb"&&" and \verb"||").

When mixing unrelated operators, though, it's a good idea to parenthesize.
C and it's friends present pernicious (恶劣的) precedence problems, and
it's easy to make a mistake. Because the logical operators bind tighter
than assignment, parentheses are mandatory for most expressions that
combine them:
\begin{wellcode}
    while ((c = getchar()) != EOF)
        ...
...
Even though it contains only one increment, this assignment can also give
varying results:
\begin{badcode}
    array[i++] = i;
\end{badcode}
...
or loop over an array runs uphill this time and downhill the next, or
strings are copied with \verb'strcpy' here and a \verb'for' loop there, the
variations make it harder to see what's really going on. But if the same
computation is done the same way every time it appears, any variation
suggests a genuine (真正的) difference, one worth nothing.
...
idiomatically expressed as a chain of \verb'if .. else if .. else', like
this:
% the "condition." and "statement." should be italic.
\begin{wellcode}
    if (condition1)
        statement1
    else if (condition2)
...
\emph{Avoid function macros.} In C++, \verb'inline' functions render (使得)
...
parameter that appears more than once in the definition might be
...
It's always better to use the \verb'ctype' function than to implement them
...
outright (明显的) error. Consider this example:
\begin{badcode}
...
By giving names to the principal (主要的) numbers in the calculation, we
...
\emph{Use character constants, not integers.} The functions in
...
The \verb'sizeof' operator is sometimes a convenient way to avoid inventing
...
\emph{Don't belabor (痛打) the obvious.} Comments shouldn't report
...
Global variables have a tendency to crop up (突然出现) intermittently
...
\emph{Don't comment bad code, rewrite it.} Comment anything unusual or
...
Comments should not only agree with code, they should support it. The
...
Students are taught that it's important to comment everything. Professional
...
has been carelessly tossed (折腾) and never polished. In the rush (匆忙) to
get programs out the door to met some deadline, it's easy to push style
aside, to worry about it later. This can be a costly decision. Some of the
...
The key observation is that good style should be a matter of habit. If you
...
common with writing good English. Strunk and White's \bookname{The Elements
    of Style} (Allyn \& Bacon) is still the best short book on how to write
English well.

This chapter draw on the approach of \bookname{The Elements of Programming
Style} by Brian Kernighan and P. J. Plauger (McGraw-Hill, 1978). Steve
Maguire's \bookname{Writing Solid Code} (Microsoft Press, 1993) is an
excellent source of programming advise. There are also helpful discussions
of style in Steve McConnell's \bookname{Code Complete} (Microsoft Press,
1993) and Peter van der Linder's \bookname{Expert C Programming: Deep C
