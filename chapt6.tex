...
that a program is broken. Testing is a determined (决定了的),
...
create tests from compact (简洁的) specifications.
...
complexity:
\begin{itemize}
    \item search an array with no elements
...
\end{wellcode}
This is simpleminded (纯朴的)
but it shows that a useful test scaffold need not be
...
all print \verb'yes' and the last two tests should print \verb'no'.
...
verify correct operation. Whenever the program is changed, even in a
...
value before the modulus (模数). Without this test,
